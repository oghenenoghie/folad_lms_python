...
def test_link_and_unlink_guardian(
    ...
    listed = api_client.get(f"/api/v1/student-guardians?student_id={student.public_id}")
    assert listed.status_code == 200
    assert listed.json()["data"]["pagination"]["total_count"] == 1

    duplicate = api_client.post(
        "/api/v1/student-guardians",
        {"student": str(student.public_id), "guardian": str(guardian.public_id), "relationship_type": "father"},
        format="json",
    )
    assert duplicate.status_code == 409

    unlinked = api_client.delete(f"/api/v1/student-guardians/{link_id}")
...
def test_guardian_link_rejects_cross_tenant_guardian(
    ...
    user = user_factory(organization=organization, email="a@example.com", password="s3cret-pass!")
    _grant(user, "student_guardians.create")
    _login(api_client, "a@example.com", "s3cret-pass!")
