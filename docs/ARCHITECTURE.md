...
3. **PostgreSQL RLS (defence-in-depth).** On connection checkout, the app issues `SET app.current_org = <org_id>`. RLS policies on tenant tables (`USING (organization_id = current_setting('app.current_org')::bigint)`) reject cross-tenant rows even if a query forgets the app-layer filter or an SQL-level bug slips through. SUPER_ADMIN platform queries run under a role exempted from RLS, used only in audited platform code.
